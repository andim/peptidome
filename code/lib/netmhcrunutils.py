...
netmhcpanpath = os.path.join(repopath, "dependencies/netMHCpan-4.1/netMHCpan")


def run_netMHC(inputpath, outname, hla, binder_only=True, overwrite=False):
    ...
    if (not os.path.exists(fullout)) or overwrite:
        # netMHC -f fasta -xls -xlsfile out.csv -a HLA-A0101
        print(f"{netmhcpath} -f {inputpath} -xls -xlsfile {fullout} -a {hla} -p")
        if peptides:
            subprocess.run(
                [
                    netmhcpath,
