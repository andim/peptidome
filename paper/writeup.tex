...
\section{Linking likelihood and neighbor density}
...
We expect sequences that are highly likely under a particular model to also have more neighbors in a set of sequences drawn from that same model. Can we make this link quantitative? It turns out we can as we show in the following. For mathematically tractability we focus on an independent site model (a first order model, which as discussed before actually does a rather good job at describing peptide statistics). As before we denote $k$ the length of the sequences $\sigma$ we will consider and $S$ the number of possible choices per site. For peptides we have of course $S=20$ amino acids. For generality we do not impose that the model is translation invariant, i.e. the probability of an amino acid can depend on position,
\begin{equation} \label{eqpsigma}
    p(\B \sigma) = \prod_{i=1}^k p_i(\sigma_i).
\end{equation}
Let us define the probability that a randomly drawn sequence is a neighbor of sequence $\sigma$ as
\begin{equation}
