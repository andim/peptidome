...
We also show the divergences with respect to a uniform distribution. We find that natural peptides are generally largely easier to differentiate from random peptides than from peptides from a different proteome. This points towards underlying shared biases in amino acid usage among the different proteomes.
...
We can also use our theory to quantify under which conditions a quorum sensing strategy can work, i.e. a decision-making strategy relying not on the presence of a single peptide, but $k$ different peptides, simultaneously present on a cell or in a tissue site. While in practice there are likely constraints on how reliably different cells can communicate their assessment and coordinate their decision, here we neglect these constraints to put a theoretical upper bound on the performance of such a quorum sensing mechanism on a purely statistical basis. To do so we calculate the distribution of cumulative likelihood ratios of $k$ randomly sampled peptides from either the pathogen or host proteome. We then determine the achievable false negative rate by a decision rule that maintains a given low false positive rate. We find that more than a hundred peptides are needed for reliable detection of an infected cell (Fig.~\ref{figquorum}). However, this large number can be greatly reduced when all responses are focused on a subset of most discriminable peptides: if only the top 10\% of peptides are detected than the number of peptides drops substantially to $~30$. 

\begin{figure}
    \begin{center}
    \includegraphics{quorum_requirement}
    \end{center}
    \caption{How many peptides would need to be considered for reliable quorum sensing? Achievable false negative rates at a fixed false positive rate $=10^{-3}$ depend on the number of independent samples and which peptides the decision are based on (recall). Focusing on the top discriminable peptides decreases numbers needed substantially, but of course also lowers the number of peptides available for classification substantially.
    \label{figquorum}
    }
\end{figure}
